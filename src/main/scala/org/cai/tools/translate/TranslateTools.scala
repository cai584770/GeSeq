...
object TranslateTools {
  ...
  def translateFirst(ba: Array[Byte]): StringBuilder = {
    var cur = 0
    var temp1: Byte = 0
    var temp2: Byte = 0
    ...
    ba.foreach { byte =>
      ...
      if (cur == 0) {
        protein.append(Encoding.encodingProteinFirst(unit1, unit2, unit3))
        cur = 1
        temp1 = unit4
      } else if (cur == 1) {
        protein.append(Encoding.encodingProteinFirst(temp1, unit1, unit2))
        cur = 2
        temp1 = unit3
        temp2 = unit4
      } else if (cur == 2) {
        protein.append(Encoding.encodingProteinFirst(temp1, temp2, unit1))
        protein.append(Encoding.encodingProteinFirst(unit2, unit3, unit4))
...
//  }
  ...
  def translate(ba: Array[Byte]):mutable.StringBuilder  = {
    ...
    // Process byte array
    for (byte <- ba) {
      val unit1 = ((byte >> 6).toByte & 0x03).toByte
      val unit2 = ((byte >> 4).toByte & 0x03).toByte
      val unit3 = ((byte >> 2).toByte & 0x03).toByte
      val unit4 = (byte.toByte & 0x03).toByte

      cur match {
        case 0 =>
          proteinBuilder.append(Encoding.encodingProtein(unit1, unit2, unit3))
          cur = 1
          temp1 = unit4

        case 1 =>
          proteinBuilder.append(Encoding.encodingProtein(temp1, unit1, unit2))
          cur = 2
          temp1 = unit3
          temp2 = unit4

        case 2 =>
          proteinBuilder.append(Encoding.encodingProtein(temp1, temp2, unit1))
          proteinBuilder.append(Encoding.encodingProtein(unit2, unit3, unit4))
          cur = 0
      }
    }
